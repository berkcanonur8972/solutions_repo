...
# Function to plot a graph with resistance labels
def plot_circuit(G, title, pos=None):
...
G1.add_edges_from([('A', 'C', {'weight': 2}), ('C', 'B', {'weight': 3}), ('A', 'B', {'weight': 5})])
plot_circuit(G1, "Initial Circuit Graph")

# Step 2: After Series Reduction
G2 = nx.Graph()
G2.add_nodes_from(['A', 'B'])
G2.add_edges_from([('A', 'B', {'weight': 5}), ('A', 'B', {'weight': 5})])  # Two edges for parallel
plot_circuit(G2, "After Series Reduction (A-C-B combined)")

# Step 3: After Parallel Reduction
G3 = nx.Graph()
G3.add_nodes_from(['A', 'B'])
G3.add_edge('A', 'B', weight=2.5)
plot_circuit(G3, "After Parallel Reduction (Final Equivalent Resistance)")
```
